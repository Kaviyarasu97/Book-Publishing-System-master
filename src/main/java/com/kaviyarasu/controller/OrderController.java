...
import com.kaviyarasu.service.OrderService;
...
public class OrderController {
	...
	public String myOrders(ModelMap modelMap, HttpSession session) {
		...
		List<Order> list = orderService.findByUserIdOrderByIdDesc(loggedInUser.getUserId());
	...
	public String updateStatus(@RequestParam("id") int orderId , @RequestParam("status")String status ) {
		...
		if ("CANCELLED".equals(status) ) {
			...
			order.setDeliveredDay(LocalDate.now());
		}
